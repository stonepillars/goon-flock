/image/arrest_tag_hover
	plane = PLANE_NOSHADOW_ABOVE
	maptext_x = -64
	maptext_y = -5 - 8 // -7 to accomodate extra text (-5?)
	maptext_width = 160
	maptext_height = 48
	icon = null
	appearance_flags = PIXEL_SCALE

	//proc/set_tag(name, extra)
	//	src.maptext = {"<span class='pixel c ol'><span style='font-size: 6px;'>[name]</span><br><span style='font-size: 5px;'>[extra]</span></span>"}
	proc/set_tag(name, sechud_flag)
		src.maptext = {"<span class='pixel c ol'><span style='font-size: 6px;'>[name]</span><br><span style='font-size: 5px;'>[sechud_flag]</span></span>"}
		//src.maptext = {"<span class='pixel c ol'><span style='font-size: 6px;'>[name]</span>"}

/atom/movable/arrest_tag
	appearance_flags = TILE_BOUND | RESET_COLOR | RESET_ALPHA | RESET_TRANSFORM | KEEP_APART | PIXEL_SCALE
	alpha = 180
	icon = null
	mouse_opacity = 0

/atom/movable/arrest_tag/outer
	var/image/arrest_tag_hover/hover_image
	var/cur_name = null

	New()
		..()
		hover_image = new(null, src)
		get_image_group(CLIENT_IMAGE_GROUP_ARREST_ICONS).add_image(hover_image)

	disposing()
		get_image_group(CLIENT_IMAGE_GROUP_ARREST_ICONS).remove_image(hover_image)
		dispose(hover_image)
		..()

	proc/set_tag(name, sechud_flag)
		if (name != src.cur_name)
			src.hover_image.set_tag(name, sechud_flag)
			src.cur_name = name
